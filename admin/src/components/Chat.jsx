...
const Chat = ({ appointmentId, userType, currentUserId, userName, otherUserName, onClose, onChatClosed, onEndChat }) => {
    ...
    useEffect(() => {
        ...
        const newSocket = io(backendUrlFinal, {
            auth: {
                token: token
            },
            extraHeaders: {
    ...
    const fetchMessages = async () => {
        try {
            const { data } = await axios.get(
                `${backendUrlFinal}/api/chat/messages/${appointmentId}`,
    ...
    const uploadFile = async () => {
        ...
        try {
            ...
            const { data } = await axios.post(
                `${backendUrlFinal}/api/chat/upload-file`,
                formData,
                {
                    headers: {
                        dToken: token,
    ...
    const handleEndChat = async () => {
        try {
            ...
            if (data.success) {
                setChatClosed(true);
                if (socket) {
                    socket.emit('chat_closed', { chatId: appointmentId });
                }
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl h-[80vh] flex flex-col">
                ...
                <div className="bg-primary text-white p-4 rounded-t-lg flex justify-between items-center">
                    <div>
                        ...
                        {chatClosed && (
                            <p className="text-sm text-red-200">Chat closed by doctor</p>
                        )}
                    </div>
                    <div className="flex gap-2">
                        {userType === 'doctor' && !chatClosed && (
                            <button
